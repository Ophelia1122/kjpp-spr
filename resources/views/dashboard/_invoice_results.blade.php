    ...
    <div class="bg-white rounded-b-lg border border-gray-200 shadow-sm dark:bg-gray-800 dark:border-gray-700">
        ...
        <div class="flex flex-wrap items-center justify-between gap-3 border-b border-gray-100 px-4 py-2.5 dark:border-gray-700">
            <p class="text-sm text-gray-500 dark:text-gray-400">{{ $invoices->total() }} invoice ditemukan</p>
                ...
                @forelse ($invoices as $inv)
                    ...
                    <tr data-href="{{ route('proposals.show', $inv->project) }}"
                        ...
                        <td class="px-4 py-3 text-xs text-gray-500 whitespace-nowrap dark:text-gray-400">
                            ...
                            <div class="{{ $inv->payment_date ? 'text-emerald-600 dark:text-emerald-400' : 'text-gray-400 dark:text-gray-500' }}">
        ...
        <div class="divide-y divide-gray-100 md:hidden dark:divide-gray-700">
                ...
                <div data-href="{{ route('proposals.show', $inv->project) }}"
                    ...
                    <div class="flex flex-wrap items-center justify-between gap-2">
                        <div class="flex flex-wrap items-center gap-x-2 gap-y-1 text-xs">
                            ...
                            @else
                                <span class="{{ $isOverdue ? 'font-semibold text-rose-600 dark:text-rose-400' : 'text-gray-400 dark:text-gray-500' }}">{{ $isOverdue ? '⚠ ' : '' }}{{ $inv->age_days }} hari</span>
                            @endif
                        </div>
                            ...
                            @can('invoices.manage')
                                @if (!$isPaid && !$isCancelled)
                                    <button type="button" title="Tandai Dibayar"
                                            onclick="openMarkPaidModal('{{ route('invoices.markAsPaid', $inv) }}', {{ \Illuminate\Support\Js::from($inv->invoice_number . ' — ' . $rp($inv->amount)) }})"
                                            class="grid h-8 w-8 place-items-center rounded-md text-gray-500 hover:bg-emerald-100 hover:text-emerald-700 dark:text-gray-400 dark:hover:bg-emerald-900/30 dark:hover:text-emerald-300">
                                        <svg class="h-[18px] w-[18px]" fill="none" viewBox="0 0 24 24" stroke-width="1.7" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5"/></svg>
                                    </button>
                                @endif
                            @endcan
                            @can('invoices.view')
            ...
            @empty
                <p class="px-4 py-10 text-center text-sm text-gray-400 dark:text-gray-500">Belum ada invoice yang cocok dengan filter ini.</p>
            @endforelse
        </div>
    </div>

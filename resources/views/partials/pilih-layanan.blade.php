...
@php
    ...
    foreach (\App\Models\Project::CONSULTING_TYPES as $jenis) {
        $pilihan->push([
            'grup'  => 'Konsultasi',
            'judul' => $jenis,
            'sub'   => \App\Models\Project::CONSULTING_HINTS[$jenis] ?? '',
            'url'   => route('proposals.create', ['layanan' => \App\Models\Project::SERVICE_KONSULTASI, 'jenis' => $jenis]),
            'ikon'  => match ($jenis) {
                \App\Models\Project::CONSULTING_RAB => 'M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 0 0 2.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 0 0-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 0 0 .75-.75 2.25 2.25 0 0 0-.1-.664m-5.8 0A2.251 2.251 0 0 1 13.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25ZM6.75 12h.008v.008H6.75V12Zm0 3h.008v.008H6.75V15Zm0 3h.008v.008H6.75V18Z',
                \App\Models\Project::CONSULTING_FS  => 'M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 0 1 3 19.875v-6.75ZM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V8.625ZM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V4.125Z',
                default => 'm21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z',
            },
        ]);
    }
@endphp
...
<div id="pilihLayananModal" class="fixed inset-0 z-[70] hidden items-center justify-center bg-gray-900/60 p-4"
    ...
    <div class="w-full max-w-lg rounded-xl bg-white p-6 shadow-2xl dark:bg-gray-800">
        ...
        @foreach ($pilihan->groupBy('grup') as $grup => $isi)
            <p class="mt-4 text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ $grup }}</p>
            <div class="mt-1.5 space-y-1.5">
                @foreach ($isi as $item)
                    <a href="{{ $item['url'] }}"
                       class="flex items-start gap-3 rounded-lg border border-gray-200 px-3 py-2.5 text-left transition hover:border-blue-400 hover:bg-blue-50 dark:border-gray-700 dark:hover:border-blue-700 dark:hover:bg-blue-900/20">
                        <span class="mt-0.5 grid h-8 w-8 shrink-0 place-items-center rounded-md bg-gray-100 text-gray-600 dark:bg-gray-900 dark:text-gray-300">

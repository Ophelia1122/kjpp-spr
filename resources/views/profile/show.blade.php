...
<div class="max-w-2xl mx-auto py-8 space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            ...
            <p class="text-sm text-gray-500">{{ $user->email }} · {{ $user->role->name ?? '-' }}</p>
        </div>
    </div>

    {{-- ===================== BIODATA ===================== --}}
    <form action="{{ route('profile.update') }}" method="POST" class="space-y-4 bg-white rounded-lg border border-gray-200 shadow-sm p-6 lift">
        @csrf
        @method('PUT')

        <h2 class="text-sm font-semibold text-gray-500 uppercase tracking-wide">Biodata</h2>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
            <div>
                ...
                <span class="block text-xs text-gray-400">Diatur oleh Administrator.</span>
            </div>
            <div>
                <span class="block text-gray-500">Status Akun</span>
    ...
    {{-- ===================== KEAMANAN ===================== --}}
    <form action="{{ route('profile.updatePassword') }}" method="POST" class="space-y-4 bg-white rounded-lg border border-gray-200 shadow-sm p-6 lift">
        @csrf
        @method('PUT')

        <h2 class="text-sm font-semibold text-gray-500 uppercase tracking-wide">Keamanan — Ganti Password</h2>

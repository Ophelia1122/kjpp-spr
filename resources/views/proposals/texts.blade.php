...
<div class="max-w-4xl mx-auto py-8 space-y-6">
    ...
    <div class="flex items-start justify-between gap-4">
        <div>
            <h1 class="text-xl font-semibold text-gray-900">Editor Teks Proposal per-Bab</h1>
            <p class="text-sm text-gray-500">{{ $project->proposal_number }} — {{ $project->instructingClient->client_name }}</p>
        </div>
        <a href="{{ route('proposals.show', $project) }}" class="text-sm text-gray-500 hover:text-gray-700 whitespace-nowrap">&larr; Kembali ke Detail Proyek</a>
    </div>
        ...
        @foreach ($sections as $i => $section)
            <div id="bab-{{ $section['key'] }}"
                 class="scroll-mt-6 bg-white rounded-lg border shadow-sm p-5
                        {{ $section['overridden'] ? 'border-amber-300' : 'border-gray-200' }}">

                <div class="flex items-start justify-between gap-3">
                    <h2 class="text-sm font-semibold text-gray-900">
                        <span class="text-gray-400 font-normal">{{ $i + 1 }}.</span>
                        {{ $section['title'] }}
                    </h2>
                    @if (! $section['editable'])
                        <span class="shrink-0 px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-500 border border-gray-200">
                            Otomatis
                        </span>
                    @elseif ($section['overridden'])
                        <span class="shrink-0 px-2 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-800 border border-amber-300">
                            Diedit
                        </span>
                    @endif
                </div>
                ...
                @if (! $section['editable'])
                    <p class="mt-3 text-sm text-gray-400 italic">Bab ini dibuat otomatis dari data proyek — tidak ada teks untuk diedit.</p>
                @else
                    ...
                    <form action="{{ route('proposals.texts.update', [$project, $section['key']]) }}" method="POST" class="mt-3">
                        ...
                        <textarea name="body" rows="{{ $rows }}"
                                  class="w-full rounded-md border-gray-300 shadow-sm text-sm font-mono leading-relaxed"
                        ...
                        <div class="mt-2 flex flex-wrap items-center gap-3">
                            ...
                            @if ($section['overridden'])
                                <span class="text-xs text-amber-700">Bab ini memakai teks hasil edit.</span>
                            @endif
                        </div>
                    </form>
                    ...
                    @if ($section['overridden'])
                        <div class="mt-3 pt-3 border-t border-gray-100 flex flex-wrap items-center gap-4">
                            ...
                            <details class="text-xs text-gray-500">
                                <summary class="cursor-pointer hover:text-gray-700">Lihat teks baku terkini</summary>
                                <pre class="mt-2 whitespace-pre-wrap font-mono bg-gray-50 border border-gray-200 rounded p-3 text-gray-600">{{ $section['baku'] }}</pre>
                            </details>
                        </div>
                    @endif
                @endif
